...
def gas(channel, parameter):
    channels[channel] = parameter
    send_control_packet()
    time.sleep(0.01)
    for q in range(GasNum):
        parameter += Gas
        channels[channel] = parameter
        send_control_packet()
        time.sleep(0.01)
    ...
    time.sleep(0.03)
    for q in range(BrakeNum):
        parameter -= Brake
        channels[channel] = parameter
        send_control_packet()
        time.sleep(0.01)
        
def brake(channel, parameter):
    channels[channel] = parameter
    send_control_packet()
    time.sleep(0.01)
    for q in range(GasNum):
...
##The amount of increasement or decreasement can not be too less or too many
##Too less makes the drone non-moving
##Too many makes the drone moving too fast and the slow reaction of commands
